...
public class Option extends BuildObject implements IOption, IBuildPropertiesRestriction {
	...
	private List<String> aliases = null; // what this options was named in older versions; CUSTOMIZATION
	...
	protected void loadFromManifest(IManagedConfigElement element) {
        ...
        //<CUSTOMIZATION>
        String aliasesString = element.getAttribute(ALIASES);
        if (aliasesString != null){
            aliases = new ArrayList<String>();
            for (String s: aliasesString.split(",")){
                s = s.trim();
                if (s.length() > 0)
                    aliases.add(s);
            }
...
*/
    ...
    public String[] getAliases(){  //CUSTOMIZATION
        if (aliases != null) return aliases.toArray(new String[aliases.size()]);
        return null;
    }
    ...
    public String getBaseId(){
        if (this.isExtensionElement() || this.superClass == null) return getId();
        return this.superClass.getBaseId();
    }
