...
public class CSourceNotFoundEditor extends CommonSourceNotFoundEditor {
	...
	protected void viewDisassembly() {		
		...
		if (page != null) {		
			//<CUSTOMIZATION>
            // See if there is a custom disassembler for this debugger.
            String id = "org.eclipse.cdt.dsf.debug.ui.disassembly.view"; //$NON-NLS-1$ ICDebugUIConstants.ID_DISASSEMBLY_VIEW;
            if (launch != null)
                try {
                    id = launch.getAttribute(ICDTLaunchConfigurationConstants.ATTR_DEBUGGER_DISASSEMBLY_VIEW_ID, id);
                }
                catch (CoreException e1) {
                    // Whatever the problem is should be manifested later.
                }
            //</CUSTOMIZATION>
			try {
				page.showView( id /*"org.eclipse.cdt.dsf.debug.ui.disassembly.view"*/); //$NON-NLS-1$
