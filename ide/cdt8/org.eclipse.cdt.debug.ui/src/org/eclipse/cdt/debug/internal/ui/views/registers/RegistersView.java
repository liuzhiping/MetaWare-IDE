...
public class RegistersView extends AbstractDebugView implements ISelectionListener, IDebugEventSetListener {
    ...
    public void handleDebugEvents (DebugEvent[] events) {
        ...
        for (DebugEvent event : events) {
            if (event.getKind() == DebugEvent.CHANGE && event.getSource() instanceof IRegister) {
            	...
            	// During a restart, we can end up not having anything selected. Fix that here.
                handleTargetSuspendEvent(event);
            }
    ...
    private void handleTargetSuspendEvent (DebugEvent event) {
    	// The suspend after a restart can leave things in an unselected state. Fix that.
        if (!fHidden && viewer != null && this.fStackFrame == null){
        	this.asyncExec(new Runnable() {
                public void run () {
                    setViewerInput(getSite().getPage().getSelection(IDebugUIConstants.ID_DEBUG_VIEW));
    ...
    private void handleTargetTerminationEvent (DebugEvent event) {
        if (event.getSource() == fTarget || event.getSource() == fThread){
            this.asyncExec(new Runnable() {
                public void run () {
                	viewer.setInput(null); // clear table
                    clearToolBar();
                    fStackFrame = null;
                    fThread = null;
                }
