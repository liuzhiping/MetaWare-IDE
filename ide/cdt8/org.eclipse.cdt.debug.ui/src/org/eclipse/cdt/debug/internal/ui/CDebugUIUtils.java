...
public class CDebugUIUtils {
	...
	//CUSTOMIZATION - added  CVariableFormat format
	/**
	 * Moved from CDebugModelPresentation because it is also used by CVariableLabelProvider.
	 */
	static public String getValueText( IValue value, CVariableFormat format ) { //CUSTOMIZATION XKORAT
		...
		if ( value instanceof ICDebugElementStatus && !((ICDebugElementStatus)value).isOK() ) {
			...
			try {
				...
				String valueString =  format != null?((ICValue)value).getValueString(format):value.getValueString();
				if ( valueString != null ) {
    ...
    static public String getValueText( IValue value ) {
        return getValueText(value,null);
    }
