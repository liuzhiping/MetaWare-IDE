...
public class CBreakpointUpdater implements ICBreakpointListener {
		...
		asyncExec( new Runnable() {
			...
			public void run() {
				try {
					...
					//<CUSTOMIZATION>
					Integer type = (Integer)attributes.get(ICBreakpointType.TYPE);
					((ICBreakpoint)breakpoint).setType(type != null?type.intValue():ICDIBreakpoint.REGULAR);
					//</CUSTOMIZATION>
	 ...
	 */
	@Override
	public void breakpointsRemoved( final IDebugTarget target, final IBreakpoint[] breakpoints ) {
		asyncExec( new Runnable() {
			...
			public void run() {
				for ( int i = 0; i < breakpoints.length; ++i ) {
					try {
						//CUSTOMIZATION: if breakpoint being removed as a side-effect
                        // of a user request, then delete the last reference.
                        // If it is being deleted as a "cleanup" operation as 
                        // a target is terminating, then don't delete it. We preserve
                        // it across sessions.
                        int cnt =((ICBreakpoint)breakpoints[i]).decrementInstallCount();
						//if ( ((ICBreakpoint)breakpoints[i]).decrementInstallCount() == 0 )
                        //CUSTOMIZATION
                        if (cnt == 0) {
                            if (!target.isTerminated()){
                                breakpoints[i].delete(); // CUSTOMIZATION
                            }
                            else 
