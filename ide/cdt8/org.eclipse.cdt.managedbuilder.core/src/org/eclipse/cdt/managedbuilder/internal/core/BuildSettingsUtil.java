	...
	public static void copyCommonSettings(ITool fromTool, ITool toTool){
		...
		for(int i = 0; i < COMMON_SETTINGS_IDS.length; i++){
			...
			for(int k = 0; k < fromOptions.length; k++){
				...
				//<CUSTOMIZATION SYNOPSYS> match the IDs in from to to
				boolean done = false;
			    for (IOption toOpt: toOptions){
			        if (toOpt.getBaseId().equals(fromOption.getBaseId())){
			            //ManagedBuildManager.setOption(toTool.getParentResourceInfo(), toTool, toOpt, v.toArray(new String[v.size()]));
			            // Will be copied over later 
			            done = true;
			            break;
			        }
			    }
        ...
        // <CUSTOMIZATION> Copy options that happen to have same IDs
        IResourceInfo rcInfo = toTool.getParentResourceInfo();
        for (IOption fromOption : fromTool.getOptions()) {
            if (!fromOption.isExtensionElement()) {
                IOption toOption = toTool.getOptionBySuperClassId(fromOption.getBaseId());
                if (toOption == null && fromOption.getSuperClass() != null) {
                	// See if we can find an option that matches one that the source is overriding
                	toOption = toTool.getOptionBySuperClassId(fromOption.getSuperClass().getBaseId());
                	if (toOption != null && fromOption.getSuperClass().getSuperClass() != null){
                        toOption = toTool.getOptionBySuperClassId(fromOption.getSuperClass().getSuperClass().getBaseId());
                	}
                }
                try {
                    if (toOption != null && toOption.getValueType() == fromOption.getValueType()) {
                        switch (toOption.getBasicValueType()) {
                        case IOption.BOOLEAN:
                            ManagedBuildManager.setOption(rcInfo, toTool, toOption,
                                    fromOption.getBooleanValue());
                            break;
                        case IOption.STRING:
                        case IOption.ENUMERATED:
                            ManagedBuildManager.setOption(rcInfo, toTool, toOption,
                                    fromOption.getStringValue());
                            break;
                        case IOption.STRING_LIST:
                            ManagedBuildManager.setOption(rcInfo, toTool, toOption,
                                    fromOption.getBasicStringListValue());
                            break;
                        default:
                            break;
