...
public class CardLayoutBuilder extends ContainerBuilder {
    ...
    static String getStringValue(Object o){
    	if (o == null) return "0";   // Punt here. It shouldn't happen.
    ...
    public Object returnObject() throws SAXException {
        ...
        if (mProperty != null) {
            _gui.addPropertyChangeListener(mProperty,
                    new PropertyChangeListener() {
                        ...
                        public void propertyChange(PropertyChangeEvent event) {
                            try {
                                mCards.showCard(getStringValue(event.getNewValue()));
                            }
                            catch (IllegalArgumentException e) {
                                //the same property may be used for multiple cardLayouts and
                                // we can have a non-existent page on a container that may
                                // not even be visible.
                            }
                        }
            ...
            if (_gui.getProperty(mProperty) != null)
                try {
                    mCards.showCard(getStringValue( _gui.getProperty(mProperty) ));
                }
                catch (IllegalArgumentException e) {
                    //the same property may be used for multiple cardLayouts and
                    // we can have a non-existent page on a container that may
