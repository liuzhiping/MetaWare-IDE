...
import org.eclipse.cdt.debug.core.model.ISteppingModeTarget2;
...
* CUSTOMIZATION: implements ISteppingModeTarget2
*/
public class CDebugTarget extends CDebugElement implements ICDebugTarget, ISteppingModeTarget2, ICDIEventListener, ILaunchListener, IExpressionListener, ISourceLookupChangeListener, IStep {
//public class CDebugTarget extends CDebugElement implements ICDebugTarget, ICDIEventListener, ILaunchListener, IExpressionListener, ISourceLookupChangeListener {
     ...
     * Set if start symbol could not be found. Thus it will not be reset on restart.
     */
    private boolean fStopSymbolNotFound = false;
	...
	public CDebugTarget(ILaunch launch, IProject project, ICDITarget cdiTarget, String name, IProcess debuggeeProcess, IBinaryObject file, boolean allowsTerminate, boolean allowsDisconnect) {
		...
		getCDISession().getEventManager().addEventListener(this);
		 ...
		 //<CUSTOMIZATION> if target bombed during startup, then don't lose the termination event.
        if (cdiTarget.isTerminated()){
            terminated();
        }
        //</CUSTOMIZATION>
	}
	...
	public void start(String stopSymbol, boolean resume) throws DebugException {
		...
		//<CUSTOMIZATION>
        ICDITarget t = getCDITarget();
        if (t instanceof ICDITarget3){
            try {
                ((ICDITarget3)t).start();
                if (t.isTerminated()) return; // License request failure?
            }
		...
		if (config.supportsBreakpoints()) {
			...
			if (stopSymbol != null && stopSymbol.length() != 0 
					&& getAddressFactory() != null /*CUSTOMIZATION*/) {
				...
				try {
					...
					//CUSTOMIZATION
					if (!stopAtSymbol( stopSymbol )){
                        resume = false;
                        fStopSymbolNotFound = true;
                    }
				}
			}
		}
        ...
        if (t instanceof ICDITarget3) {
            try {
                ((ICDITarget3) t).onBreakpointsRestored();
            }
            catch (CDIException e) {
                targetRequestFailed(e.getMessage(), null);
            }
		...
		if (config.supportsResume() && resume) {
            ...
            refreshThreads(); // CUSTOMIZATION
            fireSuspendEvent( DebugEvent.UNSPECIFIED ); //CUSTOMIZATION
        }
	}
	...
	protected void initializeThreads(List<DebugEvent> debugEvents) {
		...
		for (int i = 0; i < cdiThreads.length; ++i) {
			...
			try {
				//CUSTOMIZATION
				//if (cdiThreads[i].equals(cdiTarget.Thread()) && thread.isSuspended()) {
	...
	//CUSTOMIZATION
    private boolean hasSuspendedThread(){
        for (CThread thread: (List<CThread>)getThreadList()){
    ...
    //CUSTOMIZATION
    private boolean hasResumedThreadExceptFor(ICDIThread exceptFor){
        for (CThread thread: (List<CThread>)getThreadList()){
            if (((CThread)thread ).getCDIThread() != exceptFor && !thread.isSuspended()) return true;
        }
        return false;
    }
    ...
    public boolean isAnimating(){
        if (this.fCDITarget instanceof IAdaptable){
            ICDIAnimatable a = (ICDIAnimatable)((IAdaptable)this.fCDITarget).getAdapter(ICDIAnimatable.class);
            return a != null && a.isAnimating();
        }
        return false;
    }
	...
	public boolean canResume() {
		...
		return getConfiguration().supportsResume() && 
		        (hasSuspendedThread() || getState() == CDebugElementState.SUSPENDED &&
		            getThreadList().size() == 0); //CUSTOMIZATION
	}
	...
	public boolean canSuspend() {
		if (!getConfiguration().supportsSuspend())
			return false;
		if (getState().equals(CDebugElementState.RESUMED) || getState().equals(CDebugElementState.ANIMATING)) { // CUSTOMIZATOIN
			// only allow suspend if no threads are currently suspended
			// CUSTOMIZATION: correction: allow if any thread is not suspended.
			IThread[] threads = getThreads();
			for (int i = 0; i < threads.length; i++) {
				//CUSTOMIZATION
				//if (threads[i].isSuspended()) {
				//	return false;
				if (!threads[i].isSuspended() || ((CThread) threads[i]).isAnimating())
                    return true;
				
			}
			// return true;
            // If no threads are yet created, but the process is in a resumed state, then
            // return true so that we can stop it.
            if (threads.length == 0)
                return true;
		}
		return false;
	...
	public void resume() throws DebugException {
		...
		//CUSTOMIZATION - DAVID_REVIEW - do you remove changeState to be setState
		//changeState(newState);
		setState( newState );
		//<CUSTOMIZATION>
        // If some of the threads are already resumed, their state will not change.
        Iterator it = getThreadList().iterator();
        while( it.hasNext() ) {
            CThread c = (CThread)it.next();
            if (c.getState() != CDebugElementState.RESUMED)
                c.setState( newState );
        }
        //<CUSTOMIZATION>
	...
	public void suspend() throws DebugException {
		...
		//CUSTOMIZATION  DAVID_REVIEW changeState ?
		//changeState(newState);
		setState(newState);
		//<CUSTOMIZATION>
        Iterator it = getThreadList().iterator();
        while( it.hasNext() ) {
            CThread next = (CThread)it.next();
            // Only those threads that are not already suspended will be affected.
            if (next.getState() != CDebugElementState.SUSPENDED)
    ...
    public Object getAdapter(@SuppressWarnings("rawtypes") Class adapter) {
		...
		/*<CUSTOMIZATION>*/
        if ( adapter.equals( IThread.class ) || adapter.equals( ICThread.class)){
	...
	public void handleDebugEvents(ICDIEvent[] events) {
		for (int i = 0; i < events.length; i++) {
			...
			//CUSTOMIZATION: gdb's stuff terminates a target by setting the source to null.
            // We don't do that.
            if ( (source == null || source.equals(cdiTarget)) && event instanceof ICDIDestroyedEvent ) {
				...
				if (event instanceof ICDICreatedEvent) {
					...
					//<CUSTOMIZATION>
                        ...
                        // Thread suspending
                        handleSuspendedThreadEvent( (ICDISuspendedEvent) event); 
                    }
                    //<//CUSTOMIZATION>
				}
				 ...
				 //<CUSTOMIZATION>
                        ...
                        // Thread resuming
                        handleResumedThreadEvent( (ICDIResumedEvent) event); 
                    }
                    //<//CUSTOMIZATION>
				}
	...
	public void restart() throws DebugException {
		...
		//CUSTOMIZATION DAVID_REVIEW ?
		boolean resume = true;
		//</CUSTOMIZATION>
		try {
			ILaunchConfiguration launchConfig = getLaunch().getLaunchConfiguration();
			//CUSTOMIZATION on resume and condition if resume && !fStopSymbolNotFound 
			resume = launchConfig.getAttribute(ICDTLaunchConfigurationConstants.ATTR_DEBUGGER_RESUME_AT_START, ICDTLaunchConfigurationConstants.DEBUGGER_RESUME_AT_STARTUP_DEFAULT);
					
			if (resume && !fStopSymbolNotFound && launchConfig.getAttribute(ICDTLaunchConfigurationConstants.ATTR_DEBUGGER_STOP_AT_MAIN, ICDTLaunchConfigurationConstants.DEBUGGER_STOP_AT_MAIN_DEFAULT)) {
	...
	private void handleSuspendedEvent(ICDISuspendedEvent event) {
		...
		if (event.getSource() instanceof ICDITarget) {
			 //CUSTOMIZATION -move the below to function supportsThreadControl
	...
	// <CUSTOMIZATION>
    private void handleResumedThreadEvent( ICDIResumedEvent event ) {
        // If even a single thread resumes, put the process in a resumed state.
        if (getState() != CDebugElementState.RESUMED && supportsThreadControl())
            handleResumedEvent(event);
    }
    // </CUSTOMIZATION>
    
    // <CUSTOMIZATION>
    private void handleSuspendedThreadEvent( ICDISuspendedEvent event ) {
        // If all threads are now suspended, then so is the target.
        if (getState() != CDebugElementState.SUSPENDED && supportsThreadControl() &&
                !hasResumedThreadExceptFor((ICDIThread)event.getSource()))
            handleSuspendedEvent(event);
    }
    // </CUSTOMIZATION>
    
	private void handleResumedEvent(ICDIResumedEvent event) {
		if (!isAnimating()) //CUSTOMIZATION
	...
	/**
     * Return whether or not we can control each individual thread of the target. Otherwise, we assume
     * we can only handle things at the target level.
     * <P>
     * CUSTOMIZATION
     * @return whether or not we can control each individual thread of the target. 
     */
    private boolean supportsThreadControl () {
        return (this.getConfiguration() instanceof ICDITargetConfiguration2) && ((ICDITargetConfiguration2)this.getConfiguration()).supportsThreadControl();
    }
    //</CUSTOMIZATION>
    
	private void handleEndSteppingRange(ICDIEndSteppingRange endSteppingRange) {
		//<CUSTOMIZATION>
		if (isAnimating()) setState(CDebugElementState.ANIMATING);
		//</CUSTOMIZATION>
		fireSuspendEvent(DebugEvent.UNSPECIFIED);
	}
		...
		if (cdiTarget instanceof ICDITarget2) {
			...
			if (hasCDIGlobals) {
				for (int i = 0; i < cdiGlobals.length; i++) {
					...
					//<CUSTOMIZATION>
                    String path = null;
                    if (cdiGlobals[i] instanceof ICDIGlobalVariableDescriptor2){
                        path = ((ICDIGlobalVariableDescriptor2)cdiGlobals[i]).getPath();
                    }
                   //</CUSTOMIZATION>
	...
	public boolean isInstructionSteppingEnabled() {
		//CUSTOMIZATION - include checking fPreferences
		return fPreferences != null && fPreferences.getBoolean( PREF_INSTRUCTION_STEPPING_MODE )
				/*|| //CUSTOMIZATION: prevent instruct stepping from being permanent
	...
	public IAddressFactory getAddressFactory() {
		if (fAddressFactory == null) {			
			...
			if (fAddressFactory == null) {
				//CUSTOMIZATION -no need to check for project here
				// so this can be used for no project case
				if (getExecFile() != null ) {
					IBinaryObject file;
					file = getBinaryFile();
					if (file != null) {
		...
		try {
			...
			if (cdiTarget != null) {
				...
				if (cdiTarget instanceof ICDITarget3){
	                  //<CUSTOMIZATION>
	                    ArrayList<String> mapList = new ArrayList<String>(containers.length);
	                    getDirectoryMappingPaths(mapList,containers);
	                    ((ICDITarget3)cdiTarget).setDirectoryTranslation(mapList.toArray(new String[mapList.size()]));
	...
	//CUSTOMIZATION
    private void getDirectoryMappingPaths( List<String> list, ISourceContainer[] containers ) {
        for ( int i = 0; i < containers.length; ++i ) {
            if ( containers[i] instanceof MapEntrySourceContainer ) {
                MapEntrySourceContainer m = (MapEntrySourceContainer)containers[i];
                list.add(m.getBackendPath().toString());
                list.add(m.getLocalPath().toOSString());
            }
            if ( containers[i].isComposite() ) {
                try {
                    getDirectoryMappingPaths( list, containers[i].getSourceContainers() );
                }
                catch( CoreException e ) {
	...
	private void getSourceLookupPath(LinkedHashSet<String> list, ISourceContainer[] containers) {
		for (ISourceContainer container : containers) {
			...
			if (container instanceof CProjectSourceContainer) {
				...
				//CUSTOMIZATION
				if ( folderContainer != null && folderContainer.exists() 
					    && folderContainer.getLocation() != null) {   //CUSTOMIZATION: check for null; occurs from link
	 ...
	 * @param symbol
	 */
	//CUSTOMIZATION return boolean
	private boolean stopAtLocation(ICDILocation location, String stopExpression) throws DebugException {
		try {
			setInternalTemporaryBreakpoint(location);
			return true;//CUSTOMIZATION
		} catch (CoreException e) {
			final ICDITarget cdiTarget = getCDITarget();
	...
	// CUSTOMIZATION
    private CThread getCurrentCThread(){
        ICDIThread current;
        try {
            current = getCDITarget().getCurrentThread();
        }
        catch (CDIException e) {
            return null; // presumably caught later
        }
        return current == null? null:findThread(current);
    }

    // CUSTOMIZATION
    public boolean canStepInto () {
        if (!hasSteppingCapability()) return false;
        CThread c = getCurrentCThread();
        return c != null && c.canStepInto();
    }
     ...
     * @return
     */
    private boolean hasSteppingCapability () {
        return this.fThreads != null && (this.fThreads.size() == 1 || !supportsThreadControl());
    }
 ...
 // CUSTOMIZATION
    public boolean canStepReturn () {
        if (!hasSteppingCapability()) return false;
        CThread c = getCurrentCThread();
        return c != null && c.canStepReturn();
    }

 // CUSTOMIZATION
    public boolean isStepping () {
        CThread c = getCurrentCThread();
        return c != null && c.isStepping();
    }
