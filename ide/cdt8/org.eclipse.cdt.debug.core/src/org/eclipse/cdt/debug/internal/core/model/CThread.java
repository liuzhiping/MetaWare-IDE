...
public class CThread extends CDebugElement implements ICThread, IRestart, IResumeWithoutSignal, ICDIEventListener {
	...
	protected synchronized List computeStackFrames( boolean refreshChildren ) throws DebugException {
		if (isSuspended() || isAnimating()) { // CUSTOMIZATION: check for animation
		//if ( isSuspended() ) {
			if ( isTerminated() ) {
				fStackFrames = new ArrayList();
			}
    ...
    public boolean isAnimating () {
        if (this.fCDIThread instanceof IAdaptable) {
            ICDIAnimatable a = (ICDIAnimatable) ((IAdaptable) this.fCDIThread).getAdapter(ICDIAnimatable.class);
	...
	public boolean canResume() {
		...
		return ( fConfig.supportsResume() && isSuspended() && !isAnimating());
	}
	...
	public boolean canSuspend() {
		...
		return ( fConfig.supportsSuspend() &&
			    (state == CDebugElementState.RESUMED || state == CDebugElementState.STEPPED  ||
			     state == CDebugElementState.ANIMATING) ); //CUSTOMIZATION
	}
	...
	public boolean isSuspended() {
		...
		return getState() == CDebugElementState.SUSPENDED;
	}
	...
	protected boolean canStep() {
		...
		//<CUSTOMIZATION>
		if (!this.isCurrent() && fConfig instanceof ICDITargetConfiguration2 &&
		    !((ICDITargetConfiguration2)fConfig).supportsThreadControl())
		    return false;
		//</CUSTOMIZATION>
	...
	private void handleSuspendedEvent( ICDISuspendedEvent event ) {
		if ( !(getState().equals( CDebugElementState.RESUMED ) || 
			   ...
			   getState().equals( CDebugElementState.RESUMING ) || //CUSTOMIZATION
			   getState().equals( CDebugElementState.ANIMATING ) || //CUSTOMIZATION
			   getState().equals( CDebugElementState.SUSPENDING )) )
			return;
		ICDISessionObject reason = event.getReason();
		
		//<CUSTOMIZATION>
		if (!isAnimating() || !(reason instanceof ICDIEndSteppingRange)) {
	...
	private void handleResumedEvent( ICDIResumedEvent event ) {
		...
		if ( isCurrent() && event.getType() != ICDIResumedEvent.CONTINUE ) {
			...
			state = isAnimating()?CDebugElementState.ANIMATING:CDebugElementState.STEPPED; //CUSTOMIZATION
	...
	private void handleEndSteppingRange( ICDIEndSteppingRange endSteppingRange ) {
		 ...
		 //<CUSTOMIZATION>
        if (isAnimating()){
            setState(CDebugElementState.ANIMATING);
        }
        //</CUSTOMIZATION>
	...
	public Object getAdapter( Class adapter ) {
		...
		//<CUSTOMIZATION>
        if (adapter == ICDIJumpToLine.class){
            if (getCDIThread() instanceof ICDIJumpToLine){
                return getCDIThread();
            }
            if (getCDIThread() instanceof IAdaptable){
                Object o = ((IAdaptable)getCDIThread()).getAdapter(ICDIJumpToLine.class);
                if (o != null) return o;
            }
        }
        //</CUSTOMIZATION>
	...
	protected void resumedByTarget( int detail, List events ) {
		...
		if ( isCurrent() && detail != DebugEvent.CLIENT_REQUEST && detail != DebugEvent.UNSPECIFIED ) {
			...
			setState(  isAnimating()?CDebugElementState.ANIMATING:CDebugElementState.STEPPED );
			preserveStackFrames();
			events.add( createResumeEvent( detail ) );
		}
	    else if (detail != DebugEvent.STEP_INTO && detail != DebugEvent.STEP_OVER) { /*CUSTOMIZATION*/
