...
public class CRegister extends CVariable implements ICRegister {
	...
	private class InternalVariable implements IInternalVariable {
		...
		//CUSTOMIZATION
        public synchronized void setChildChanged(){
            setChanged(true);
        }
    ...
    public Object getAdapter (Class klass) {
        ...
        if (o == null) {
            InternalVariable var = (InternalVariable)this.getOriginal();
            ICDIRegister reg;
            try {
                reg = var.getCDIRegister();
            }
            catch (DebugException e) {
                return null;
            }
            // Most likely converting to IRawValue
            if (reg instanceof IAdaptable)
                return ((IAdaptable)reg).getAdapter(klass);
        }
        return o;
    }
}
