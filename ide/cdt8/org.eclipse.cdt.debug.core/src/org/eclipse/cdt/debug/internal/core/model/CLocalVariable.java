	...
	private class InternalVariable implements IInternalVariable {
		...
		public void invalidateValue() {
			if ( fValue instanceof AbstractCValue ) {
				// CUSTOMIZATION: Eclipse 3.2 framework feature:
				// If variable has elements and the stack frame doesn't
				// change, then those elements are continued to be referenced
				// in the tree view regardless if the variable has changed.
				// Therefore, do not dispose of elements when invalidating
				if (!(fValue instanceof CIndexedValue)) {
		...
		public synchronized void setChanged( boolean changed ) {
			...
			//CUSTOMIZATION
			//if ( fValue instanceof AbstractCValue ) {
			if (fValue instanceof AbstractCValue && (!changed ||
					// We may see an array change, but this does not mean every element
					// changed.
							/* CUSTOMIZATION */!(fValue instanceof CIndexedValue))) {
				((AbstractCValue)fValue).setChanged( changed );
			}
			fChanged = changed;
			//<CUSTOMIZATION> DAVID_REVIEW as no tag on 7.0
			if (changed) {
				CDebugElement parent = getParent();
				if (parent instanceof CVariable) {
					((CVariable) parent).setChildChanged();
				} else if (parent instanceof AbstractCValue) {
					parent = ((AbstractCValue) parent).getParentVariable();
					if (parent instanceof CVariable) {
						((CVariable) parent).setChildChanged();
					}
				}
		...
		public synchronized void setChildChanged(){ //CUSTOMIZATION
	        fChanged = true;
	    }
