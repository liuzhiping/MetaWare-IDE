...
public class CValue extends AbstractCValue {
	...
	/**
	 * Cached format. Must recompute value string if changed.
	 * CUSTOMIZATION
	 */
	private CVariableFormat fValueFormat = CVariableFormat.NATURAL;
	...
	public String getValueString() throws DebugException {
		...
		if ( (fValueString == null || fValueFormat != format)&& getUnderlyingValue() != null ) {
			resetStatus();
			...
			if ( isSuspended ) {
				try {
					fValueString = processUnderlyingValue( getUnderlyingValue() , format); //CUSTOMIZATION XKORAT
				}
	...
	//CUSTOMIZATION
	private String processUnderlyingValue( ICDIValue cdiValue, CVariableFormat format) throws CDIException {
		if ( cdiValue != null ) {
			if ( cdiValue instanceof ICDIBoolValue )
				return getBoolValueString( (ICDIBoolValue)cdiValue, format );
			if ( cdiValue instanceof ICDICharValue )
				return getCharValueString( (ICDICharValue)cdiValue, format );
			else if ( cdiValue instanceof ICDIShortValue )
				return getShortValueString( (ICDIShortValue)cdiValue,format );
			else if ( cdiValue instanceof ICDIIntValue )
				return getIntValueString( (ICDIIntValue)cdiValue, format );
			else if ( cdiValue instanceof ICDILongValue )
				return getLongValueString( (ICDILongValue)cdiValue, format );
			else if ( cdiValue instanceof ICDILongLongValue )
				return getLongLongValueString( (ICDILongLongValue)cdiValue, format );
			else if ( cdiValue instanceof ICDIBigIntegerValue )
				return getBigIntegerValueString( (ICDIBigIntegerValue)cdiValue, format );
			else if ( cdiValue instanceof ICDIFloatValue )
				return getFloatValueString( (ICDIFloatValue)cdiValue, format );
			else if ( cdiValue instanceof ICDIDoubleValue )
				return getDoubleValueString( (ICDIDoubleValue)cdiValue, format );
			else if ( cdiValue instanceof ICDIPointerValue )
				return getPointerValueString( (ICDIPointerValue)cdiValue, format );
			else if ( cdiValue instanceof ICDIReferenceValue )
				return processUnderlyingValue(((ICDIReferenceValue)cdiValue).referenceValue(), format);
			else if ( cdiValue instanceof ICDIWCharValue )
				return getWCharValueString( (ICDIWCharValue)cdiValue, format );
			else
				return  getGenericValueString(cdiValue.getValueString(), format);
		}
		return null;
	}
	...
	//CUSTOMIZATION -input CVariableFormat format
	private String getBoolValueString( ICDIBoolValue value, CVariableFormat format ) throws CDIException {
		//CVariableFormat format = getParentVariable().getFormat(); 
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getCharValueString( ICDICharValue value, CVariableFormat format ) throws CDIException {
		//CVariableFormat format = getParentVariable().getFormat(); 
		if ( CVariableFormat.NATURAL.equals( format ) ) {
			...
			*****************************************************************/
			...
			if (v < 0 || v > 127) return Integer.toString(v) + " (=0x" + Integer.toHexString(v) + ")";
			...
			switch (v){
			    ...
			    default:
			        if (Character.isISOControl((char)v)) {
			            if ( v <= 10) return Integer.toString(v);
			            return Integer.toString(v) + " (=0x" + Integer.toHexString(v) + ")";
			        }
			        sb.append('\'');
			        if (v == '\'' || v == '\\'){
			            sb.append('\\');
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getShortValueString( ICDIShortValue value, CVariableFormat format ) throws CDIException {
		//CVariableFormat format = getParentVariable().getFormat();
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getIntValueString( ICDIIntValue value, CVariableFormat format ) throws CDIException {
		//CVariableFormat format = getParentVariable().getFormat();
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getLongValueString( ICDILongValue value, CVariableFormat format ) throws CDIException {
		try {
			//CVariableFormat format = getParentVariable().getFormat();
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getLongLongValueString( ICDILongLongValue value, CVariableFormat format ) throws CDIException {
		try {
			//CVariableFormat format = getParentVariable().getFormat();
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getGenericValueString(String svalue, CVariableFormat format) throws CDIException {
		try {
	...
	//CUSTOMIZATION CVariableFormat format
	private String getFloatValueString( ICDIFloatValue value, CVariableFormat format ) throws CDIException {
	...
	//CUSTOMIZATION CVariableFormat format
	private String getDoubleValueString( ICDIDoubleValue value, CVariableFormat format ) throws CDIException {
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getPointerValueString( ICDIPointerValue value, CVariableFormat format ) throws CDIException {
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getWCharValueString( ICDIWCharValue value, CVariableFormat format ) throws CDIException {
	...
	//CUSTOMIZATION to add CVariableFormat format
	private String getBigIntegerValueString( ICDIBigIntegerValue value, CVariableFormat format ) throws CDIException {
		try {
			//CVariableFormat format = getParentVariable().getFormat();
	...
	private static CVariableFormat getNaturalFormat(ICDIValue value, CVariableFormat defaultFormat) throws CDIException {
	 ...
	 public String getValueString(CVariableFormat format) throws DebugException {
	         if (getUnderlyingValue() != null){
	                 try {
	                         return processUnderlyingValue(getUnderlyingValue(),format);
	                 } catch (CDIException e) {
	                         setStatus( ICDebugElementStatus.ERROR, e.getMessage() );
