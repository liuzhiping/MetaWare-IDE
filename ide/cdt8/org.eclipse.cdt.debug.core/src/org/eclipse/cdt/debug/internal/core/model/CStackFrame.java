...
public class CStackFrame extends CDebugElement implements ICStackFrame, IRestart, IResumeWithoutSignal, IMoveToAddress, IMoveToLine, ICDIEventListener {
	...
	protected synchronized List getVariables0() throws DebugException {
		...
		if ( thread.isSuspended() || thread.isAnimating() /*CUSTOMIZATION*/) {
			if ( fVariables == null ) {			
	...
	public void handleDebugEvents( ICDIEvent[] events ) {
		//<CUSTOMIZATION>
	    // Handle change event that the target fires if the source location suddenly changed, due to, say,
	    // stepping into an inlined function.
	    for (ICDIEvent e: events){
	        if (e instanceof ICDIChangedEvent && e.getSource() == fCDIStackFrame){
	...
	public boolean isSuspended() {
		//CUSTOMIZATION
		 CThread thread = (CThread)getThread();
	    // CUSTOMIZATION: test for animating
		return getThread().isSuspended() || thread.isAnimating();	
	}
	...
	private synchronized CExpression getExpression( String expressionText ) throws DebugException {
		...
		try {
			...
			expression = new CExpression( this, cdiExpression, null );
			 ...
			 //<CUSTOMIZATION> Make expression format persistent
            IWatchExpression e = ExpressionFormatManager.getInstance().findWatchExpression(expressionText);
            if (e != null){
                CVariableFormat format = ExpressionFormatManager.getInstance().getFormat(e);
                if (format != CVariableFormat.NATURAL){
	...
	public int getOffsetFromFunction () {
	        ICDILocator loc = this.fCDIStackFrame.getLocator();
	        if (loc instanceof ICDIFunctionLocation2){
	            return ((ICDIFunctionLocation2)loc).getOffsetFromFunction();
	        }
	        return 0;
	}
}
