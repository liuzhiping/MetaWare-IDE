...
public class TypeFactory implements ITypeFactory {
    ...
    public IType createInteger(String name, int size, boolean isUnsigned) {
        ...
        if (size == 1){
            ...
            if (name.toLowerCase().indexOf("bool") >= 0) {
                if (mBoolType == null)
                    mBoolType = new BooleanType(name,1,mTarget);
                return mBoolType;
            }
    ...
    @Override
    public IType createBoolean(String name, int size){
        if (mBoolType == null)
            mBoolType = new BooleanType(name,size,mTarget);
        return mBoolType;
    }
