...
class SeeCodeOptionsAugmenter implements ISeeCodeOptionsAugmenter {
    ...
    public String[] augmentArguments(String[] arguments) {
        ...
        boolean isSimulationTarget = false;
        boolean isOtherTarget= false;
        for ( int i = 0 ; i <len; i ++){
        	// This check is added during fixing CR:9000576155
        	// TN: It looks like this check helps to fix for the case if target is virtualizer or xcam etc...
        	// so that there will be no other compiler options to be transferred 
        	// However there is case that user launched the app by short cut way, we need to check for isOtherTargets
        	
            if(arguments[i].equals("-sim") ||arguments[i].equals("-nsim"))
            	isSimulationTarget = true;
       	
            // continue searching //CR 9000624754 since user used short cut where they didnot launch Debug Configuration,
            // we should allow to addArguments as the compiler selection implied Simulation type
            // failure to do this cause licensing searching problem for particular local node license
            if(arguments[i].equals("-hard") ||arguments[i].equals("-virtualizer") || (arguments[i].indexOf("arccli") >=0) || (arguments[i].equals("-rascal")))
            	isOtherTarget = true;
     	
        }
        // if arguments not contain target info - it is because user launches from short cut
        if(isSimulationTarget || (!isSimulationTarget && !isOtherTarget))
           return addArgumentsFromBuildConfiguration(arguments);
        // this case is for isOtherTargets = true
        return arguments;
    }
    ...
    public void augmentDefaults (Map<String, Object> propertyMap) {
        for (DefaultSpec spec: mDefaultList){
            ...
            if (spec.optionValue.equals(value)){
                propertyMap.put(spec.property,spec.value);
                break; // So as to override subsequent "!" settings
            }
        }
    }
