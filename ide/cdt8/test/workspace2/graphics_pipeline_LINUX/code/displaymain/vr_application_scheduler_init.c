...
{
  if (seq == 0)
    return swarch_thd_displaymain_thread_id;
...
{
  if (!strcmp ("thd_displaymain", thread_name))
    return swarch_thd_displaymain_thread_id;
...
vr_app_init (void)
{
  ...
  if (!strcmp (vr_get_thread_name (), "thd_displaymain"))
    {
      return 0;
    }
...
vr_app_shutdown (char * thread_name)
{
  ...
  if (!strcmp (thread_name, "thd_displaymain"))
    {
      return;
    }
...
{
  /*ARC: getCpuInit() */
  /* displaymain is a slave process so wait for barrier */
  vr_nc_store((void *)(&vr_barrier[1]), 1);
  while (vr_nc_load((void *)(&vr_barrier[1])) == 1) { (void)vr_sleep(0, 10); }
...
{
  char * default_arguments[] = {"./displaymain", "thread", "0.0", "vr-svr", "thd_displaymain", "--no-malloc-fallback" };
