...
#if EVENTB_DELAY
	  Loadfunction(EVENTB_DELAY);
#endif

      event_result = _event_wait_all(Event1_handle, (_mqx_uint)0x1, (_mqx_uint)NO_TIMEOUT);
      if (event_result != MQX_OK) {
         /* waiting on event failed */
      } /*Endif*/
      /* clear the event bits after processing event */
      event_result = _event_clear(Event1_handle, (_mqx_uint)0x1);
   } /* Endwhile */ 
} /*EndBody*/
