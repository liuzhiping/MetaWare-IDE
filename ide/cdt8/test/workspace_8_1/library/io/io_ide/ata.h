...
*** File: ata.h
***
*** Comments: This file is added to address CR 2283     
***    
***
**************************************************************************
*END*********************************************************************/
...
/*
** Byte offsets to ATA register set from hard disk drive exist on AA4 base
*/	
#define ATA_REG_BASE          (0x00)
#define ATA_DATA_BUF          (0x00)
#define ATA_ERROR             (0x04)   /* read only */
#define ATA_FEATURES          (0x04)   /* write only */
#define ATA_SECTOR_COUNT      (0x08)
#define ATA_LBA_LOW           (0x0C)
#define ATA_LBA_MID           (0x10)
#define ATA_LBA_HIGH          (0x14)
#define ATA_DEVICE_LBA        (0x18)
#define ATA_STATUS            (0x1C)   /* read only: reading clears interrupt */
#define ATA_COMMAND           (0x1C)   /* write only */
#define ATA_ALT_STATUS_CMD    (0x38)   /* reading does NOT clear interrupt */
#define ATA_DEV_CNTRL         (0x38)   /* write only */
...
#define ATA_DEVICE_FLUSH_CACHE                     (0xE7)
...
#define ATA_DRIVE_REG_OBSOLETE  (0xA0)
...
#define ATA_ERR_MASK \
   ( ATA_ERR_CRC | ATA_ERR_UNC | ATA_ERR_IDNF | ATA_ERR_ABRT | \
     ATA_ERR_TKONF | ATA_ERR_AMNF )

/*
** Defines for the ATA device control register
*/
#define ATA_DEV_CNTL_SRST (0x00000004)
...
/* The maximum time to wait for the ready bit to set in seconds */
#define ATA_RTY_TIMEOUT               (31)
...
*/
typedef volatile struct ata_identify_device_struct {
   uint_16     GENERAL_CONFIG;                   /* Word 0 */
   uint_16     NUMBER_CYLINDERS;                 /* Word 1 */
   uint_16     NUMBER_REMOVABLE_CYLINDERS;       /* Word 2 */
   uint_16     NUMBER_HEADS;                     /* Word 3 */
   uint_16     RESERVED1[2];                     /* Word 4-5 */
   uint_16     SECTORS_PER_TRACK;                /* Word 6 */
   uint_16     BYTES_SECTOR_GAP;                 /* Word 7 */
   uint_16     BYTES_IN_SYNC;                    /* Word 8 */
   uint_16     RESERVED2;                        /* Word 9 */
   uint_16     SERIAL_NUMBER[10];                /* Word 10-19 */
   uint_16     CONTROLLER_TYPE;                  /* Word 20 */
   uint_16     BUFFER_SIZE;                      /* Word 21 */
   uint_16     NUMBER_ECC_BYTES;                 /* Word 22 */
   uint_16     MICROCODE_VERSION[4];             /* Word 23-26 */
   uint_16     MODEL_NUMBER[20];                 /* Word 27-46 */
   uint_16     VENDOR_MAX_SEC_NUM;               /* Word 47 */
   uint_16     RESERVED3;                        /* Word 48 */
   uint_16     CAPABILITIES;                     /* Word 49 */
   uint_16     CAPABILITIES_BIT_ASSIN;           /* Word 50 */
   uint_16     PIO_TIMING_MODE;                  /* Word 51 */
   uint_16     DMA_TIMING_MODE;                  /* Word 52 */
   uint_16     VALIDITY_FLAG;                    /* Word 53 */
   ...
   uint_16     RESERVED4[6];                     /* Word 69-74 */
   uint_16     QUEUE_DEPTH;                      /* Word 75 */
   uint_16     RESERVED5[3];                     /* Word 76-79 */
   uint_16     MAJOR_VERSION_NUMBER;             /* Word 80 */
   uint_16     MANOR_VERSION_NUMBER;             /* Word 81 */
   uint_16     COMMAND_SET_SUPPORT[6];           /* Word 82-87 */
   uint_16     ULTRA_DMA_MODE;                   /* Word 88 */
   uint_16     SECURITY_ERASE_TIME_REQ;          /* Word 89 */
   uint_16     ENHANCE_SECURITY_ERASE_TIME_REQ;  /* Word 90 */
   uint_16     POWER_MANAGEMENT;                 /* Word 91 */
   uint_16     RESERVED6[35];                    /* Word 92-126 */
   uint_16     REMOVABLE_MEDIA_STATUS_FEATURE;   /* Word 127 */
   uint_16     DEVICE_LUCK_FUNCTION;             /* Word 128 */
   uint_16     FEATURE_OPTION;                   /* Word 129 */
   uint_16     VENDOR[30];                       /* Word 130-159 */
   uint_16     RESERVED7[96];                    /* Word 160-255 */
