...
{ /* Body */
   ...
   if (result == MQX_OK) {
      ...
      if ((_mqx_uint)flags & IO_SERIAL_HW_FLOW_CONTROL) {
         ...
         (*int_io_dev_ptr->DEV_IOCTL)(int_io_dev_ptr->DEV_INFO_PTR,
            IO_IOCTL_SERIAL_SET_HW_SIGNAL, &ioctl_val);
      } /* Endif */
      return (result);
   } /* Endif */
...
{ /* Body */
   ...
   if (--int_io_dev_ptr->COUNT == 0) {
      if (fd_ptr->FLAGS & IO_SERIAL_HW_FLOW_CONTROL) {
         ioctl_val = IO_SERIAL_RTS;
         (*int_io_dev_ptr->DEV_IOCTL)(int_io_dev_ptr->DEV_INFO_PTR,
            IO_IOCTL_SERIAL_CLEAR_HW_SIGNAL, &ioctl_val);
      } /* Endif */
...
{ /* Body */
   ...
   while (i--) {
      if (flags & IO_SERIAL_TRANSLATION) {
         if (*data_ptr == '\n') {
            _io_serial_int_putc_internal(int_io_dev_ptr, '\r');
         } /* Endif */
      } /* Endif */
      _io_serial_int_putc_internal(int_io_dev_ptr, *data_ptr++);
   } /* Endif */
   return num;
...
void _io_serial_int_putc_internal
   (
...
{ /* Body */
   ...
   /* Start CR 2264 */
   /* _taskq_resume if !_CHARQ_FULL, 
      not just if _CHARQ_EMPTY */
   if (!_CHARQ_FULL(int_io_dev_ptr->OUT_QUEUE)) {
      /* output queue has room */ 
