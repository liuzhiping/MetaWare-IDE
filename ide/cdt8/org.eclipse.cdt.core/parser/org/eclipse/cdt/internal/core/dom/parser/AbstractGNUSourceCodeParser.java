...
import org.eclipse.cdt.internal.core.dom.parser.c.CASTAlias;
...
public abstract class AbstractGNUSourceCodeParser implements ISourceCodeParser {
	...
	//CUSTOMIZATION
    private static boolean looksLikeExpression(IToken token)
	{
	...
	protected final IASTExpression castExpression(CastExprCtx ctx, ITemplateIdStrategy strat) throws EndOfFileException, BacktrackException {
		//if (LT(1) == IToken.tLPAREN) {
		 // CUSTOMIZATION: check for lookLike to avoid deep recursion for "(((((1+...)..."
        if (LT(1) == IToken.tLPAREN && !looksLikeExpression(LA(2))) {
	...
	protected IASTDeclaration functionStyleAsmDeclaration() throws BacktrackException, EndOfFileException {
    	...
    	IASTFunctionDefinition funcDefinition = nodeFactory.newFunctionStyleAsmDeclaration(declSpec, (IASTFunctionDeclarator)fdtor, cs);
    	//</CUSTOMIZATION>
	...
	/**
	 * Parse High C's iterator for-loop statement.
	 * <P>
	 * CUSTOMIZATION
	 * @return the AST for the statement.
	 */
	protected IASTStatement parseForIteratorStatement() throws EndOfFileException, BacktrackException {
	    int startOffset = consume().getOffset();
	    List<IASTSimpleDeclaration> controlVars = new ArrayList<IASTSimpleDeclaration>();
	    IASTName name = identifier(); 
	    // The search mechanism wants to see a declaration. So contrive one.
	    while (true) {
	    	IASTDeclSpecifier spec = nodeFactory.newSimpleDeclSpecifier();
	        IASTSimpleDeclaration declaration = nodeFactory.newSimpleDeclaration(spec);
	        IASTDeclarator declarator = nodeFactory.newDeclarator(name);	        
	        declaration.setDeclSpecifier(spec);
	        spec.setParent(declaration);
	        declarator.setName(name);
	        name.setParent(declarator);
	        name.setPropertyInParent(IASTDeclarator.DECLARATOR_NAME);
	        ((ASTNode) declarator).setOffsetAndLength((ASTNode) name);
	        declaration.addDeclarator(declarator);
	        declarator.setParent(declaration);
	        ((ASTNode) declaration).setOffsetAndLength((ASTNode) name);
	        ((ASTNode) spec).setOffsetAndLength((ASTNode) name);
	        controlVars.add(declaration);
	        if (LT(1) == IToken.tCOMMA && (LT(2) == IToken.tIDENTIFIER || LT(2) == IToken.t_typename)) {
	            consume();
	            name = identifier();
	        }
	        else break;
	    ...
	    consume(IToken.tLT);
	    consume(IToken.tMINUS);
	    IASTExpression call = this.expression();
	    consume(IToken.t_do);
	    IASTStatement body = statement();
	    IASTForLoopStatement for_statement = nodeFactory.newForLoopStatement(controlVars, call, body);
	    ((ASTNode)for_statement).setOffsetAndLength(startOffset,calculateEndOffset(body)-startOffset);
	    return for_statement;
	}
	
	/**
	 * Parse MetaWare's "_Typeof<symbol>"
	 * CUSTOMIZATION>
	 */
	protected IASTExpression parseMetaWareTypeOf()throws EndOfFileException, BacktrackException {
		int startOffset = consume().getOffset();
		consume(IToken.tLT);
		IToken id = consume(IToken.tIDENTIFIER);
		IASTName name = nodeFactory.newName(id.getCharImage());
		setRange(name,id.getOffset(),id.getEndOffset());
		IASTIdExpression expr = nodeFactory.newIdExpression(name);
		setRange(expr,id.getOffset(),id.getEndOffset());
		name.setParent(expr);

		int endOffset = consume(IToken.tGT).getEndOffset();
		IASTExpression result = buildUnaryExpression(IGNUASTUnaryExpression.op_typeof, expr, 
	...
	/**
	 * CUSTOMIZATION
	 * Parse's MetaWare's pragma statement: "_Pragma name(expr1,...)"
	 * All we care about is "alias": _Pragma alias(name1,name2[,...)
	 * that makes name1 resolve to name2.
	 * @throws EndOfFileException 
	 * @throws BacktrackException 
	 */
	protected IASTAlias parsePragma() throws EndOfFileException, BacktrackException {
		int startOffset = consume().getOffset();
		IToken id = consume(IToken.tIDENTIFIER);
		IASTExpression parms = null;
		if (LT(1) == IToken.tLPAREN){
			consume(IToken.tLPAREN);
		...
		int endOffset = consume(IToken.tSEMI).getEndOffset();
		if (id.getImage().equals("alias") && parms instanceof IASTExpressionList){ //$NON-NLS-1$
			IASTExpression[] list = ((IASTExpressionList)parms).getExpressions();
			if (list.length >= 2 && list[0] instanceof IASTIdExpression &&
					list[1] instanceof IASTIdExpression){
				CASTAlias a = new CASTAlias(((IASTIdExpression)list[0]).getName(),
										    ((IASTIdExpression)list[1]).getName());
				setRange(a, startOffset, endOffset);
				return a;
