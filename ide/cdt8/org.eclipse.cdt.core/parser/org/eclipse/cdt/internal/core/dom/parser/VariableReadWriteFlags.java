...
public abstract class VariableReadWriteFlags {
	...
	protected int rwAnyNode(IASTNode node, int indirection) {
		...
		//<CUSTOMIZATION>
		else if (parent instanceof IASTRegisterInitializer) {
			return rwInRegisterInitializer((IASTRegisterInitializer) parent, indirection);
		}
		//</CUSTOMIZATION>
	...
	//<CUSTOMIZATION>
	protected int rwInRegisterInitializer(IASTRegisterInitializer parent, int indirection) {
		IASTNode grand= parent.getParent();
		if (grand instanceof IASTDeclarator) {
			IBinding binding= ((IASTDeclarator) grand).getName().getBinding();
			if (binding instanceof IVariable) {
	...
	protected int rwInInitializerList(IASTInitializerList parent, int indirection) {
		IASTNode grand= parent.getParent();
		//<CUSTOMIZATION>
		if ( (grand instanceof IASTEqualsInitializer) || (grand instanceof IASTRegisterInitializer) ) {
