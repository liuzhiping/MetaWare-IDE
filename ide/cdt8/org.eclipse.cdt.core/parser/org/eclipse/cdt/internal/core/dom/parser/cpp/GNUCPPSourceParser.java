...
import org.eclipse.cdt.core.dom.ast.IASTRegisterInitializer;	//CUSTOMIZATION
...
public class GNUCPPSourceParser extends AbstractGNUSourceCodeParser {
    ...
    private IASTExpression expression(final ExprKind kind, final BinaryExprCtx ctx, IASTInitializerClause expr, ITemplateIdStrategy strat) throws EndOfFileException, BacktrackException {
    	...
    	castExprLoop: for(;;) {
    		...
    		case IToken.tASSIGN:
    			//CUSTOMIZATION:  deal with x=>y  in argument expression
    			if (kind == ExprKind.eAssignment){
    				if (LT(2) == IToken.tGT){
    					consume();consume();
    					//DAVID_REVIEW
    					return expression(kind,ctx,expr,null);
    				}
    			}
    			/*$FALL-THROUGH$*/
	...
	protected IASTExpression unaryExpression(CastExprCtx ctx, ITemplateIdStrategy strat) throws EndOfFileException, BacktrackException {
        ...
        //<CUSTOMIZATION>
        //STAR 9000661271 address of label 
        case IToken.tAND:
            return unaryExpression(IASTUnaryExpression.op_amper, ctx, strat);
        //<CUSTOMIZATION>    
        case IToken.tPLUS:
	...
	protected IASTDeclaration declaration(DeclarationOptions option) throws EndOfFileException, BacktrackException {
        ...
        case IToken.t_private:
        	...
        	// CUSTOMIZATION
        case IGCCToken.t_PRAGMA:
        	IASTDeclaration a = parsePragma();
        	if (a != null) return a;
    ...
    private Decl declSpecifierSeq(final DeclarationOptions option, final boolean single) throws BacktrackException, EndOfFileException {
        ...
        try {
        	...
        	declSpecifiers: for (;;) {
        		...
        		//<CUSTOMIZATION>
                case IGCCToken.t_mw_typeof:
                	if (encounteredRawType || encounteredTypename)
                		throwBacktrack(LA(1));
                	typeofExpression= parseMetaWareTypeOf();
                	encounteredTypename= true;
                	endOffset= calculateEndOffset(typeofExpression);
                	break;
                // </CUSTOMIZATION>
	...
	protected IASTInitializer optionalInitializer(IASTDeclarator dtor, DeclarationOptions option) throws EndOfFileException, BacktrackException {
    	...
    	//<CUSTOMIZATION>
    	// == ARC Register initializer-clause
        if (lt1 == IToken.tEQUAL) {
            ...
            IASTRegisterInitializer initExpr= nodeFactory.newRegisterInitializer(initClause);
	...
	private ICPPASTFunctionDeclarator functionDeclarator(boolean isLambdaDeclarator) throws EndOfFileException, BacktrackException {
		...
		//<CUSTOMIZATION>
		// Implement High C iterators:
		// void foo(...)->(args) ...

		if (LT(1) == IGCCToken.tARROW && LT(2) == IGCCToken.tLPAREN) {
			consume();
			IASTParameterDeclaration ypd = yieldParameterDeclaration();
			endOffset = calculateEndOffset(ypd);
			fc.prependParameterDeclaration(ypd);
		}
		if (LT(1) == IGCCToken.tNOT) // e.g., "void foo()!"
			consume();
		//</CUSTOMIZATION>
    ...
    protected IASTStatement parseForStatement() throws EndOfFileException, BacktrackException {
    	...
    	//<CUSTOMIZATION>
        // Handle High C iterator for-loops:  "for id <- iterator-call() do ..."
        if ( (LT(2) == IToken.tIDENTIFIER || LT(2) == IToken.t_typename) && (LT(3) == IToken.tCOMMA || LT(3) == IToken.tLT && LT(4) == IToken.tMINUS)){
            return parseForIteratorStatement();
        }
        //</CUSTOMIZATION>
	...
	protected IASTStatement parseReturnStatement() throws EndOfFileException, BacktrackException {
	 ...
	 /**
     * Process High C iterator arguments: ->(...).
     * The next token is the left paren.
     * <P>
     * CUSTOMIZATION
     * @return the "void (*yield)(...)" parameter declaration.
     */
    protected ICPPASTParameterDeclaration yieldParameterDeclaration()
    throws BacktrackException, EndOfFileException {
        IToken current = LA(1); // Should be "("
        int off = current.getOffset();
        int endOff = current.getEndOffset();
        //Insert "void (*yield)" ahead of the the "("
        IToken voidTok = new TokenWithImage(IToken.t_void,null,off,endOff,"void".toCharArray()); //$NON-NLS-1$
        IToken lparen = new TokenWithImage(IToken.tLPAREN,null,off,endOff,"(".toCharArray()); //$NON-NLS-1$
        IToken star = new TokenWithImage(IToken.tSTAR,null,off,endOff,"*".toCharArray()); //$NON-NLS-1$
        IToken yield = new TokenWithImage(IToken.tIDENTIFIER,null,off,endOff,"yield".toCharArray()); //$NON-NLS-1$
        IToken rparen = new TokenWithImage(IToken.tRPAREN,null,off,endOff,")".toCharArray()); //$NON-NLS-1$
        voidTok.setNext(lparen);
        lparen.setNext(star);
        star.setNext(yield);
        yield.setNext(rparen);
        rparen.setNext(current);
        // Beware of "const" qualifier, as in:
        //   void foo(...)->(...) const ...
        // We've rewritten thhe stuff after the "->(" to
        // "void (*yield)(...)"
        // If "const" follows, then it will be consumed as part of the "yield".
        // Thus, look ahead and temporarily remove it if we can.
        int parenLevel = 0;
        int i = 2;
        while (LT(i) != IToken.tEOC){
            if (LT(i) == IToken.tLPAREN) parenLevel++;
            else
            if (LT(i) == IToken.tRPAREN) {
                if (parenLevel == 0)
                    break;
                parenLevel--;
            }
        ...
        IToken followingToken = LT(i) != IToken.tEOC?LA(i+1):null;
        int ttype = 0;
        if (followingToken != null) {
            ttype = followingToken.getType();
            // Place comma after "void(*yield)(...)" so that parser will stop.
            followingToken.setType(IToken.tCOMMA);
        }
        backup(voidTok);
        ICPPASTParameterDeclaration result = parameterDeclaration();
        if (followingToken != null)
            followingToken.setType(ttype); // restore
        // Make sure offsets are correct. Since we contrived the "void (*yield)" portion of the
        // declarator, the lengths are off because of the way they are presumptuously computed.
        int len = ((ASTNode)result).getLength();
        ((ASTNode)result.getDeclSpecifier()).setOffsetAndLength(off,len);
        IASTDeclarator yieldDecl = ((IASTFunctionDeclarator)result.getDeclarator()).getNestedDeclarator();
        ((ASTNode)yieldDecl).setOffsetAndLength(off,len);
        ((ASTNode)yieldDecl.getName()).setOffsetAndLength(off,len);
        ((ASTNode)yieldDecl.getPointerOperators()[0]).setOffsetAndLength(off,len);
        return result;
    }
}
