...
public class CPPSemantics {
	...
	public static void populateCache(ICPPASTInternalScope scope) {
		...
		if (parent instanceof IASTCompoundStatement) {
	    	...
	    	return;
	    	// CUSTOMIZATION 
		} else if (parent instanceof IASTForLoopStatement){
			IASTForLoopStatement forStatement = (IASTForLoopStatement) parent;
			nodes = forStatement.getControlVariables();
	...
	public static void populateCache(ICPPASTInternalScope scope, IASTNode node) {
	    ...
	    if (node instanceof ICPPASTTemplateDeclaration) {
			...
			return;
		//<CUSTOMIZATION>
        } else if( node instanceof IASTForLoopStatement ){
        IASTDeclaration[] decls = ((IASTForLoopStatement)node).getControlVariables();
		...
		}else if (declaration instanceof IASTAlias){ // CUSTOMIZATION
			ASTInternal.addName(scope,((IASTAlias)declaration).getSourceName());
		}
	}
...
//        return new CPPCompositeBinding(result);
	...
	private static IBinding resolveAmbiguities(LookupData data) throws DOMException {
		...
		if (fns.size() > 0) {
	    	...
	    	//if (obj != null) {
	    	if (obj != null 
    				&& !(obj instanceof IVariable)) // CUSTOMIZATION: extern _Typeof<func1> func2 is variable
    											    // that we'd rather resolve elsewhere.
    	    {
	...
	//DAVID_REVIEW CUSTOMIZATION? missing tag
	static private boolean isIterator (IFunction function) {
        // TEMPORARY: no place to store "iterator" bit.
        // For now just see if first argument is "yield"
		IParameter parms[];

		parms = function.getParameters();
		if (parms.length > 0 && "yield".equals(parms[0].getName())) { //$NON-NLS-1$
			IType type = parms[0].getType();
			return type == null
					|| type instanceof IPointerType
					&& ((IPointerType) type).getType() instanceof IFunctionType;
		}
	...
	private static ICPPFunction[] selectByArgumentCount(LookupData data, ICPPFunction[] functions) throws DOMException {
	    ...
	    for (ICPPFunction fn : functions) {
			if (fn != null && !(fn instanceof IProblemBinding)) {
				...
				int yieldCount = 0;
				if (isIterator(fn)){
					if (numArgs+1 == parameterTypes.length){
						yieldCount++;
						numPars--;
					}
				}
				...
				//DAVID_REVIEW
				// logic  } else if (numArgs+yieldCount < function.getRequiredArgumentCount()) shown added on 7.0
				// but not on 8.1 - this logic has NO TAG CUSTOMIZATION
				boolean ok;
	...
	private static FunctionCost costForFunctionCall(ICPPFunction fn, boolean allowUDC, LookupData data)
		...
		//DAVID_REIVEW CUSTOMIZATION - remove the final
		//final IType[] paramTypes= ftype.getParameterTypes();
		IType[] paramTypes= ftype.getParameterTypes();
		...
		// <CUSTOMIZATION> deal with iterator functions that have implicit first argument that is "yield"
		if (isIterator(fn) && paramTypes.length > argTypes.length){
			IType newParams[] = new IType[paramTypes.length-1];
			System.arraycopy(paramTypes,1,newParams,0,newParams.length);
