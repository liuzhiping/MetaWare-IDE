...
public class GNUCSourceParser extends AbstractGNUSourceCodeParser {
	...
	protected IASTInitializer optionalInitializer(IASTDeclarator dtor, DeclarationOptions options) throws EndOfFileException, BacktrackException {
    	//<CUSTOMIZATION> STAR for ARC Specific Direct Register Assignment for Variables
    	//e.g. int i == 0 => mapping i into %r0
//        if (LTcatchEOF(1) == IToken.tASSIGN || LTcatchEOF(1) == IToken.tEQUAL)	{
        //</CUSTOMIZATION>
    	if (LTcatchEOF(1) == IToken.tEQUAL) {
            final int offset= consume().getOffset();
            IASTInitializerClause initClause = initClause(false);
            IASTRegisterInitializer result= nodeFactory.newRegisterInitializer(initClause);
    ...
    private IASTExpression expression(final ExprKind kind) throws EndOfFileException, BacktrackException {
		...
		loop: while (true) {
	        ...
	        case IToken.tASSIGN:
	        	//CUSTOMIZATION: look for "x=>y" as in a call
	        	if (LT(2) == IToken.tGT){
	        		consume();consume();
	...
	private IASTDeclarator functionDeclarator(DeclarationOptions paramOption) throws EndOfFileException, BacktrackException {
        ...
        if (LT(1) == IGCCToken.tARROW && LT(2) == IGCCToken.tLPAREN) {
            consume();
            IASTParameterDeclaration pd = yieldParameterDeclaration(paramOption);
            endOffset = calculateEndOffset(pd);
            if (parameters == null)
                parameters = new ArrayList<IASTParameterDeclaration>(
...
*/
protected IASTParameterDeclaration yieldParameterDeclaration(DeclarationOptions paramOption)
throws BacktrackException, EndOfFileException {
  ...
  backup(voidTok);
  IASTParameterDeclaration result = parameterDeclaration(paramOption);
  if (followingToken != null)
