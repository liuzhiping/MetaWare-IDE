...
public class CodeFormatterVisitor extends ASTVisitor implements ICPPASTVisitor, ICASTVisitor {
	...
	public int visit(IASTInitializer node) {
		...
		try {
			//<CUSTOMIZATION>
			if (node instanceof IASTRegisterInitializer) {
				visit((IASTRegisterInitializer) node);
...
//				}
	...
	//<CUSTOMIZATION>
	//TODO: right now same as IASTEqualsInitializer, need improve
	private int visit(IASTRegisterInitializer node) {
		if (node.getPropertyInParent() == IASTInitializerList.NESTED_INITIALIZER) {
			assert false;
			// Nested initializer expression, no need to apply extra alignment
			//			node.getExpression().accept(this);
			...
			// Declaration initializer
	    	Alignment alignment= scribe.createAlignment(
	    			Alignment.DECLARATION_INITIALIZER,	//need change
	    			preferences.alignment_for_assignment,	//need change
	    			Alignment.R_INNERMOST,
	    			1,	//need change???
	    			getCurrentPosition());
