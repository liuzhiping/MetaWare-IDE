...
public class ApplicabilityCalculator implements IOptionApplicability {
    ...
    public boolean isOptionVisible (IBuildObject configuration, IHoldsOptions holder, IOption option) {
        return EMGR.isApplicableToToolChain(option);
    }
    ...
    public static void setClangBased(boolean v){
        EMGR.setClangBased(v);
    }
    
    /**
     * ARC EM4 (for example) overrides the linker mapfile option to change its default.
     * @param id
     */
    public static void setMapOptionID(String id){
        EMGR.setMapOptionID(id);
    }
}
