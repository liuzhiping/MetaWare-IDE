...
//        }
    ...
    public boolean processLine(String line) {
        ...
        // We're only interested in the first line to the High C front end or clang frontend.
        // (Normally, this will only be called for metaware, but if the "ARC Generic" is used, it could
        // be called for Clang if the target is av2hs.
        //
        // MetaWare lines have "-def" and "-dir" entries.
        // ccac emits "clangac ... -Dfoo ... -I/dir
        
        if (line.indexOf("-def") < 0 && line.indexOf(" -D") < 0) return false;
        ...
        if (line.indexOf("/clang") < 0) {
        	symbols.add("__HIGHC_VER__=207");
            symbols.add("__HIGHC__=1"); // remove it later if __HIGHC_ANSI__ is defined when -Hansi is invoked.
        }
        else
            symbols.add("__GNUC__=4"); // in case user has #ifdef __GNUC__
        ...
        while (I.hasNext()) {
            ...
            else if (token.startsWith("-D")) { // clang case
                String symbol = token.substring(2);
                if (!symbols.contains(symbol)) symbols.add(symbol);
            } else if ( (token.equals("-dir") || token.equals("-internal-isystem")) && I.hasNext()) {//$NON-NLS-1$:   MetaWare case and clang case
...
//                if (!includes.contains(nPath)) includes.add(nPath);
            ...
            else if (token.startsWith("-I")){ //clang case
    ...
    public void shutdown () {
        ...
        if (includes.size() > 0 || symbols.size() > 0) {
            ...
            if(symbols.contains("__HIGHC__=1") && symbols.contains("__HIGHC_ANSI__=1"))	//option -Hansi defines __HIGHC_ANSI__ and undefines __HIGHC__
            	symbols.remove("__HIGHC__=1");
