...
import org.eclipse.cdt.debug.core.CDebugCorePlugin; //CUSTOMIZATION XKORAT
import org.eclipse.cdt.debug.core.CDebugUtils;
import org.eclipse.cdt.debug.core.ICDIDebugger;
import org.eclipse.cdt.debug.core.ICDIDebugger2;
import org.eclipse.cdt.debug.core.ICDTLaunchConfigurationConstants;
import org.eclipse.cdt.debug.core.ICDebugConfiguration;
import org.eclipse.cdt.debug.core.IRunCommand; //CUSTOMIZATION XKORAT
import org.eclipse.cdt.debug.core.cdi.CDIException;
import org.eclipse.cdt.debug.core.cdi.ICDISession;
import org.eclipse.cdt.debug.core.cdi.ICDISessionConfiguration;  //CUSTOMIZATION XKORAT
import org.eclipse.cdt.debug.core.cdi.ICDISessionConfiguration2; //CUSTOMIZATION XKORAT
...
import org.eclipse.core.runtime.Platform; //CUSTOMIZATION XKORAT
...
public class LocalCDILaunchDelegate extends AbstractCLaunchDelegate {
	...
	private void runLocalApplication(ILaunchConfiguration config, ILaunch launch, IProgressMonitor monitor) throws CoreException {
		...
		try {
			...
			String debuggerID = config.getAttribute(ICDTLaunchConfigurationConstants.ATTR_DEBUGGER_ID, (String)null);
			IRunCommand rc = null;
			if (debuggerID != null) {
			     rc = CDebugCorePlugin.getDefault().getRunCommand(debuggerID);
			}
			if (rc != null) commandArray = rc.createCommand(config,exePath.toOSString(),arguments);
	...
	private void launchLocalDebugSession(ILaunchConfiguration config, ILaunch launch, IProgressMonitor monitor) throws CoreException {
		...
		try {
			...
			//CUSTOMIZATION for no project
			if(!config.getAttribute(ICDTLaunchConfigurationConstants.ATTR_NOPROJECT, false))
				icproject = CDebugUtils.verifyCProject(config);
			
			// For No Project, we can use exePath to get the execFile
		    if (exePath != null) 
				exeFile = verifyBinary(icproject, exePath);
...
//
//          <CUSTOMIZATION>
//          To capture errors early
            try {
                Process debugger = dsession.getSessionProcess();
                if (debugger != null) {
                    String debuggerName = "Debugger";
                    ICDISessionConfiguration scfg = dsession.getConfiguration();
                    if (scfg instanceof ICDISessionConfiguration2){
                        debuggerName = ((ICDISessionConfiguration2)scfg).getSessionProcessName();
                    }
                    IProcess debuggerProcess = DebugPlugin.newProcess(launch, debugger, renderDebuggerProcessLabel(debuggerName));
			...
			// CUSTOMIZATION
			boolean resume = config.getAttribute(ICDTLaunchConfigurationConstants.ATTR_DEBUGGER_RESUME_AT_START, ICDTLaunchConfigurationConstants.DEBUGGER_RESUME_AT_STARTUP_DEFAULT); 
			
			String stopSymbol = null;
			//CUSTOMIZATION
			if (stopInMain  && resume)
			...
			for(int i = 0; i < targets.length; i++) {
				...
				if(icproject != null) //CUSTOMIZATION
					iproject = icproject.getProject();
			    CDIDebugModel.newDebugTarget(launch,iproject, targets[i], renderTargetLabel(debugConfig), iprocess, exeFile, true, false, stopSymbol, resume /*CUSTOMIZATION*/);
			}
