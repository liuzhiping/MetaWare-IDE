...
public class ToolSettingsTab extends AbstractCBuildPropertyTab implements IPreferencePageContainer {
		...
		protected void performDefaults() {
			...
			// HACK: ARC dynamically alters the defaults to set the 
			// the real processor family in subsequent actions.
			// So, we must delay setting the selection until all tabs are
			// done so that the widgets reflect reality.
		...
		private void copyHoldsOptions(IHoldsOptions src, IHoldsOptions dst, IResourceInfo res){
			...
			//CUSTOMIZATION: have case in which op1 and op2 length are not the same.
			for(int i = 0; i < Math.min(op2.length,op1.length); i++) {
