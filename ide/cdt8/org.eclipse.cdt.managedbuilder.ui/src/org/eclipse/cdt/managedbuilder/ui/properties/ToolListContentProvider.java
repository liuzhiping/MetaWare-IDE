...
public class ToolListContentProvider implements ITreeContentProvider{
	...
	//<SYNOPSYS> CUSTOMIZATION
	private static boolean hasVisibleOption(IOptionCategory category, IConfiguration config, IHoldsOptions optHolder){
		for (Object o[]: category.getOptions(config,optHolder)){
			ITool tool = (ITool)o[0];
			IOption option = (IOption)o[1];
			if (option == null) continue; // Don't know why there are null entries, but they do exist
			if (option.getApplicabilityCalculator() == null) 
			    return true;
			if (option.getApplicabilityCalculator().isOptionVisible(config, tool, option))
				return true;
		}
		return false;
	}
	...
	private void createChildElements(ToolListElement parentElement, IConfiguration config) {
		...
		for (int i=0; i<cats.length; i++) {
			...
			if ((applicabilityCalculator == null || 
					applicabilityCalculator.isOptionCategoryVisible(config, optHolder, parent))
					&& hasVisibleOption(e.getOptionCategory(),config,optHolder)  //CUSTOMIZATION SYNOPSYS: Don't show empty category
					) {
