...
public class BuildVarListDialog extends ElementListSelectionDialog {
	...
	private Text text;
	private Text valueText; //CUSTOMIZATION
	private Label type;
	...
	protected Control createDialogArea(Composite container) {
		...
		//<CUSTOMIZATION>
		desc = new Label(c, SWT.NONE);
		desc.setFont(c.getFont());
		desc.setText("Value:");  
		desc.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		valueText = new Text(c, SWT.BORDER);
		valueText.setFont(container.getFont());
	...
	protected void handleSelectionChanged() {
		...
		if (objects.length == 1) {
			...
			//<CUSTOMIZATION>
			try {
				valueText.setText(v.getStringValue());
			} catch (CdtVariableException e) {
				valueText.setText("<" + e.getMessage() + ">"); //$NON-NLS-1$ //$NON-NLS-2$
			}
			//</CUSTOMIZATION>
