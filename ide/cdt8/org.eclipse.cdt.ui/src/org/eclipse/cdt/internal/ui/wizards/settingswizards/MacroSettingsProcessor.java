...
public class MacroSettingsProcessor extends SettingsProcessor {
	...
	protected void readSettings(ICLanguageSetting setting, Element language) throws SettingsImportExportException {
		...
		Set<String> macroNameNewList = new HashSet<String>(); //CUSTOMIZATION XKORAT
		for(Element macroElement : macrosNodes) {
			...
			macroNameNewList.add(name); //CUSTOMIZATION XKORAT
		}
		...
		//CR:9000615882
		//TN CUSTOMIZATION -Filter out duplicated name list
	    //macros.addAll(setting.getSettingEntriesList(ICSettingEntry.MACRO));
		
	    macros.addAll(filterDuplicatedNames(macroNameNewList,setting.getSettingEntriesList(ICSettingEntry.MACRO)));
		setting.setSettingEntries(ICSettingEntry.MACRO, macros);
	}
	
	private List<ICLanguageSettingEntry> filterDuplicatedNames(Set<String> macroNameNewList, List<ICLanguageSettingEntry> oldList){
		
		List<ICLanguageSettingEntry> filterList = new ArrayList<ICLanguageSettingEntry>();
		
		for ( ICLanguageSettingEntry entry:oldList){
			
			if(!macroNameNewList.contains(entry.getName()))
