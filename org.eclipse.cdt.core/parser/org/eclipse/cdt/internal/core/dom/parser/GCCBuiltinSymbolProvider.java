...
public class GCCBuiltinSymbolProvider implements IBuiltinBindingsProvider {
    ...
    private void metaware(){	 
	     ...
	     if (lang == ParserLanguage.C) {
	         ...
	         abs = new CImplicitFunction("_abs".toCharArray(), scope, functionType, theParms, false); //$NON-NLS-1$
	     }
	     else {
	         ...
	         abs = new CPPImplicitFunction("_abs".toCharArray(), scope, functionType, theParms, false); //$NON-NLS-1$
	     }
	     
	     IBinding min = doMinMax("_min"); //$NON-NLS-1$
	     IBinding max = doMinMax("_max"); //$NON-NLS-1$
	     bindings = (IBinding[]) ArrayUtil.append(IBinding.class, bindings, abs);
	     bindings = (IBinding[]) ArrayUtil.append(IBinding.class, bindings, min);
		 bindings = (IBinding[]) ArrayUtil.append(IBinding.class, bindings, max);
    }
	...
	private IBinding doMinMax(String name) {
		if (lang == ParserLanguage.C) {
			...
			return new CImplicitFunction(name.toCharArray(), scope, functionType, theParms, true);
		} else {
			IType[] parms = new IType[2];
			parms[0] = c_int;
			parms[1] = c_int;
			...
			return new CPPImplicitFunction(name.toCharArray(), scope, functionType, theParms, true);
		}
	}
