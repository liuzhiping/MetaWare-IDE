...
//						return PROCESS_CONTINUE;
	...
	static protected void createBinding(IASTName name) {
		...
		if (parent instanceof CASTIdExpression) {
			...
			binding = resolveBinding(parent);
		} else if (parent instanceof IASTAlias){    // CUSTOMIZATION: _Pragma alias(name1,name2)
			IASTAlias alias = (IASTAlias)parent;
			IScope scope = getContainingScope(name);
		    try {
				binding = lookup(scope, alias.getTargetName());
					...
					if (CVisitor.unwrapTypedefs(t1) instanceof IFunctionType) {
						...
						if (binding != null && !(binding instanceof IIndexBinding) && name.isActive()) {
							...
							if (simpleDecl.getParent() instanceof IASTForLoopStatement){
								...
								if (type != null) {
									((CVariable)binding).setType(type);
								}
							}
							//<CUSTOMIZATION>
						}
	...
	private static IASTNode getContainingBlockItem(IASTNode node) {
		...
		//if parent is something that can contain a declaration
				  ...
				  parent instanceof IASTForLoopStatement  || //CUSTOMIZATION
