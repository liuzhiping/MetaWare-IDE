...
public class CPPVisitor extends ASTQueries {
	...
	private static IBinding createBinding(IASTDeclaration declaration) {
		...
		else if (declaration instanceof IASTAlias){ // CUSTOMIZATION: _Pragma alias(source,target)
			IASTAlias alias = (IASTAlias)declaration;
			ICPPScope scope = (ICPPScope) getContainingScope(declaration);
			IBinding binding;
			try {
//				IBinding oldBinding = scope.getBinding(alias.getSourceName(), false);
//				if (oldBinding != null){
//					oldBinding..
//				}
				binding = scope.getBinding(alias.getTargetName(), false);
			} catch (DOMException e) {
				binding = e.getProblem();
			}
			return binding;
        	    ...
        	    if (SemanticUtil.getNestedType(t1, TDEF) instanceof IFunctionType) {
        	    	...
        	    	if (t1 != null && t2 != null) {
        	    		...
        	    		binding = new CPPVariable(name);
        			      //<CUSTOMIZATION>
        				if (simpleDecl.getParent() instanceof IASTForLoopStatement){
        					IType type = extractForLoopControlVariableType(simpleDecl);
        					if (type != null) {
        						((CPPVariable)binding).setType(type);
        					}
        				}
        				//<CUSTOMIZATION>
        	    	}
		...
		while (node != null) {
		    ...
		    if (node instanceof IASTDeclaration) {
				...
				if (parent instanceof IASTTranslationUnit) {
                    ...
                    return ((IASTForLoopStatement)parent).getScope();
                    //</CUSTOMIZATION>
		...
		try {
			...
			if (parent instanceof ICPPASTQualifiedName) {
				...
				if (fieldReference.isPointerDereference()) {
					...
					//<CUSTOMIZATION>
					// High C supports "auto_deref" so that "p->field" can be specified "p.field"
	...
	public static IScope getContainingScope(IASTStatement statement) {
		...
		if (parent instanceof IASTCompoundStatement) {
            ...
            scope = ((IASTForLoopStatement)parent).getScope();
            //</CUSTOMIZATION>
		} else if (parent instanceof ICPPASTSwitchStatement) {
		...
		while (parent != null) {
			if (parent instanceof IASTDeclaration) {
				...
				if (p instanceof IASTForStatement)
				    return parent;
                //<CUSTOMIZATION>
                if( p instanceof IASTForLoopStatement )
                    return parent;
                //</CUSTOMIZATION>
			...
			if (node != null) {
				...
				if (node instanceof IASTFunctionDeclarator) {
					...
					IScope s = getContainingScope(funcName);
					 //<CUSTOMIZATION> due to nested functions, keep moving up until we find Class scope
					if (s instanceof ICPPTemplateScope || s instanceof ICPPBlockScope)
