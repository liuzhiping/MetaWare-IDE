...
public class CScope implements ICScope, IASTInternalScope {
	...
	public final IBinding getBinding(IASTName name, boolean resolve, IIndexFileSet fileSet) {
	    ...
	    if (o instanceof IASTName) {
	    	...
	    	// CUSTOMIZATION: choose alias if one is involved...
	    	// Dealing with MetaWare scenario:
	    	//   extern _Typeof<_func> func;  <-- Binds "func" as a variable
	    	//   _Pragma alias(func,_func) <-- Binds "func" to function matching _func
	    	IBinding best = null;
	    	for (IASTName n: ((IASTName[]) o)) {
				...
				if (b != null) {
					if (n.getParent() instanceof IFunction) return b;  // CUSTOMIZATION: hack
					best = b;
				}
			}
	    	if (best != null) // CUSTOMIZATION
	    		return best;
	    }
	...
	protected void doPopulateCache() {
		...
		else if (scopeNode instanceof IASTForLoopStatement) {
			final IASTForLoopStatement forLoopStmt = (IASTForLoopStatement)scopeNode;
			nodes = forLoopStmt.getControlVariables();
		}
		//</CUSTOMIZATION>
