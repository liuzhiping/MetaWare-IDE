...
public abstract class CElement extends PlatformObject implements ICElement {
	...
	public IPath getPath() {
		  //<CUSTOMIZATION> -- get around bug in referencing stuff in external include files
        IResource res1 = getResource();
        if (res1 != null)
            return res1.getFullPath();
        if (fParent instanceof IIncludeReference && this instanceof ITranslationUnit) {
            IPath path = ((IIncludeReference)fParent).getPath();
            return path.append(getElementName());
        }
        //</CUSTOMIZATION>
