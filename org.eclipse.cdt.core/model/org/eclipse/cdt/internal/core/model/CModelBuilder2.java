...
public class CModelBuilder2 implements IContributedModelBuilder {
	...
	private void createDeclaration(Parent parent, IASTDeclaration declaration) throws CModelException, DOMException {
		if (declaration instanceof IASTFunctionDefinition) {
			...
			// ignore
		} else if (declaration instanceof IASTAlias){
			// CUSTOMIZATION: what to do?
			IASTAlias alias = (IASTAlias)declaration;
			IBinding binding = alias.getTargetName().resolveBinding();
			if (binding != null) {
			    IASTName src = alias.getSourceName();
			    src.setBinding(binding);
			    if (binding.getScope() instanceof IASTInternalScope){
			    	((IASTInternalScope)binding.getScope()).addName(src);
			    }
			}			
