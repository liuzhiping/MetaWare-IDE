...
public class VariableFormatActionDelegate implements IObjectActionDelegate {
	...
	protected void doAction( ICVariable[] vars ) throws DebugException {
		...
		List<IWatchExpression> wlist = null;
		for (Iterator iterator = selection.iterator(); iterator.hasNext();) {
	        Object o = iterator.next();
	        if (o instanceof WatchExpression){
	        	if (wlist == null) wlist = new ArrayList<IWatchExpression>(); //CUSTOMIZATION
	        	wlist.add((IWatchExpression)o); // CUSTOMIZATION	        	
	        	((WatchExpression)o).evaluate();
	        } 
        }
		if (wlist != null){
			ExpressionFormatManager.getInstance().setFormat(wlist.toArray(new IWatchExpression[wlist.size()]), fFormat); // CUSTOMIZATION
		}
	}
