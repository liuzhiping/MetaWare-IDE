...
public class ToggleInstructionStepModeActionDelegate extends ActionDelegate implements IViewActionDelegate, IPropertyChangeListener {
	...
	/**
	 * Return whether or not the target has a custom disassembly view.
	 * <P>
	 * CUSTOMIZATION
	 * @param target
	 * @return whether or not the target has a custom disassembly view.
	 */
	private static boolean hasCustomDisassemblyView (ISteppingModeTarget target) {
        ICDITarget t = (ICDITarget) ((IAdaptable) target).getAdapter(ICDITarget.class);
        if (t instanceof ICDITarget3) {
            return ((ICDITarget3) t).hasCustomDisassemblyView();
        }
        return false;
    }
	...
	public void run( IAction action ) {
        ...
        if ( target != null ) {
    		...
    		if ( enabled &&  target instanceof ICDebugTarget && !hasCustomDisassemblyView(target)) { //CUSTOMIZATION
				try {
					getView().getSite().getPage().showView( ICDebugUIConstants.ID_DSF_DISASSEMBLY_VIEW );
				}
				catch( PartInitException e ) {
                    //CUSTOMIZATION: don't complain; we've disabled it for SeeCode
	...
	public void selectionChanged( IAction action, ISelection selection ) {
		...
		action.setEnabled( newTarget != null && newTarget.supportsInstructionStepping() 
				   ...
				   && !(newTarget instanceof ISteppingModeTarget2 && ((ISteppingModeTarget2) newTarget).isAnimating()));  //CUSTOMIZATION
	}
