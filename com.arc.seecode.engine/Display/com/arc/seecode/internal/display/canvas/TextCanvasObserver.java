...
class TextCanvasObserver implements ITextCanvas.IObserver {
     ...
     */
    @Override
    public void onSelected(ITextCanvas canvas, int lineNumber, int column) {
        mCallback.setSelectedLine(mSquareCanvas,lineNumber,column);
        if (column < 0)
            mCallback.singleClickOccurred(mSquareCanvas,lineNumber);
    }
    ...
    public void onDoubleClick(ITextCanvas canvas, int lineNumber, int column) {
        if (column != 0 || mDoubleClicksInColumn0){
            ...
            mCallback.setSelectedLine(mSquareCanvas,lineNumber,0);
        }
    }
    /**
