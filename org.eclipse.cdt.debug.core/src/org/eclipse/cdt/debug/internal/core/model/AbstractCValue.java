	...
	public String evaluateAsExpression( ICStackFrame frame ) {
		...
		if ( parent != null ) {
			if ( frame != null && frame.canEvaluate() ) {
				try {
					//<CUSTOMIZATION>
					String expressionString = parent.getExpressionString();
					IValue value = frame.evaluateExpression(expressionString);
					if (value instanceof CValue){
						valueString = ((CValue)value).getValueString(parent.getFormat());
					}
					else
						//</CUSTOMIZATION>
				     	valueString = frame.evaluateExpressionToString( expressionString );
				}
