...
public class CDebugTarget extends CDebugElement implements ICDebugTarget, ISteppingModeTarget2, ICDIEventListener, ILaunchListener, IExpressionListener, ISourceLookupChangeListener, IStep {
    ...
    private boolean hasSuspendedThread(){
        for (IThread thread: (List<IThread>)getThreadList()){
    ...
    public boolean canSuspend () {
        if (!getConfiguration().supportsSuspend())
            return false;
        if (getState() == CDebugElementState.RESUMED || getState() == CDebugElementState.ANIMATING) { // CUSTOMIZATOIN
...
//                  <CUSTOMIZATION>
		...
		if (cdiTarget instanceof ICDITarget2)
		{
			...
			if (hasCDIGlobals)
            {
                 for (int i = 0; i < cdiGlobals.length; i++) {                  
                    ...
                    //</CUSTOMIZATION>
                    list.add(CVariableFactory.createGlobalVariableDescriptor(cdiGlobals[i].getName(), path != null?new Path(path):null));
                }
            }
		}
	...
	public boolean isInstructionSteppingEnabled() {
		return fPreferences != null && fPreferences.getBoolean( PREF_INSTRUCTION_STEPPING_MODE ) /*|| //CUSTOMIZATION: prevent instruct stepping from being permanent
	...
	private void setSourceLookupPath( ISourceContainer[] containers ) {
		...
		ICDITarget target = getCDITarget();
        try {
            if (target != null) {
                target.setSourcePaths( (String[])list.toArray( new String[list.size()] ) );
                if (target instanceof ICDITarget3){
                  //<CUSTOMIZATION>
                    ArrayList<String> mapList = new ArrayList<String>(containers.length);
                    getDirectoryMappingPaths(mapList,containers);
                    ((ICDITarget3)target).setDirectoryTranslation(mapList.toArray(new String[mapList.size()]));
                    //</CUSTOMIZATION>              
                }
            }
