...
public class BuildOptionSettingsUI extends AbstractToolSettingUI {
	...
	//CUSTOMIZATION
	// For sake of GUI tester, we need to see the option tied to the widget.
	private static void setControlName(Control control, IOption opt){
	    control.setData("option",opt); //$NON-NLS-1$
	    if (control.getData("name") == null){ //$NON-NLS-1$
	        control.setData("name",opt.getBaseId()); //$NON-NLS-1$
	    }
	}
	
	//CUSTOMIZATION
	static class MyDirectoryFieldEditor extends DirectoryFieldEditor{

        public MyDirectoryFieldEditor(String name, IOption opt, Composite parent) {
            super(name, TextProcessor.process(opt.getName()), parent);
            setControlName(this.getChangeControl(null),opt);
        }	    
	}

	   //CUSTOMIZATION
    static class MyFileFieldEditor extends FileFieldEditor{

        public MyFileFieldEditor(String name, IOption opt, Composite parent) {
        	 super(name, TextProcessor.process(opt.getName()), parent);
            setControlName(this.getChangeControl(null),opt);
        }
    ...
    //CUSTOMIZATION
    static class MyStringFieldEditorM extends StringFieldEditorM{

        public MyStringFieldEditorM(String name, IOption opt, Composite parent) {
        	 super(name, TextProcessor.process(opt.getName()), parent);
            setControlName(this.getTextControl(),opt);
        }       
    }
				...
				try {
					...
					switch (opt.getValueType()) {
						case IOption.STRING: {
							...
							switch (opt.getBrowseType()) {
								case IOption.BROWSE_DIR: {
									stringField = new MyDirectoryFieldEditor(optId, opt, fieldEditorParent); //CUSTOMIZATION
								...
								case IOption.BROWSE_FILE: {
									stringField = new MyFileFieldEditor(optId, opt, fieldEditorParent){
										/**
										 * Do not perform validity check on the file name due to losing focus,
										 * see http://bugs.eclipse.org/289448
										 */
								...
								case IOption.BROWSE_NONE: {
									final StringFieldEditorM local = new MyStringFieldEditorM(optId, opt, fieldEditorParent);
									stringField = local;
						...
						case IOption.ENUMERATED: {
							...
							BuildOptionComboFieldEditor comboEditor = new BuildOptionComboFieldEditor(optId, nameStr, 
									tooltipHoverStr, contextId, enumValidNames, sel, fieldEditorParent);
							setControlName(comboEditor.getComboControl(),opt); //CUSTOMIZATION
							fieldEditor = comboEditor;
						...
						case IOption.UNDEF_MACRO_FILES:	
						{
							...
							String tooltipHoverStr = displayFixedTip ? null : tipStr;
							FileListControlFieldEditor editor = new FileListControlFieldEditor(optId, nameStr, 
									tooltipHoverStr, contextId, fieldEditorParent, opt.getBrowseType());
							fieldEditor = editor;
							 //<CUSTOMIZATION>
						    setControlName(editor.getListControl(),opt); // for GUI tester
							if (buildPropPage.getCfg() != null) {
							    IConfiguration c = buildPropPage.getCfg();
							    if (!(c instanceof Configuration)) c = null; // Prevent Cast exception in following call.
							    editor.setConfigurationDescription(c!=null?ManagedBuildManager.getDescriptionForConfiguration(c):null);
							}
							//</CUSTOMIZATION>
							if(opt.getBrowseFilterPath() != null) {
								try {
				...
				try {
					...
					case IOption.ENUMERATED:
						if(fe instanceof BuildOptionComboFieldEditor){
							...
							name = TextProcessor.deprocess(name);  //SYNOPSYS
...
//		}
		//CUSTOMIZATION
		public TriStateBooleanFieldEditor(String name, String labelText, IOption opt, Composite parent, IHoldsOptions[] ho, int curr) {
			super(name, TextProcessor.process(opt.getName()), parent);
			holders = ho; 
			current = curr;
			button = getChangeControl(parent);
			if (displayFixedTip && isToolTipBoxNeeded()) {
				button.setData(new TipInfo(labelText,opt.getToolTip()));
				button.addListener(selectAction, tipSetListener);
			} else {
				button.setToolTipText(opt.getToolTip());
			}
			setControlName(button, opt);
			if (!opt.getContextId().equals(AbstractPage.EMPTY_STR)) 
				PlatformUI.getWorkbench().getHelpSystem().setHelp(button, opt.getContextId());
