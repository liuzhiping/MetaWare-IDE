 ...
 * Synopsys -- integration of the MetaWare debugger
 *******************************************************************************/
...
public class LocalCDILaunchDelegate extends AbstractCLaunchDelegate {
	...
	private void runLocalApplication( ILaunchConfiguration config, ILaunch launch, IProgressMonitor monitor ) throws CoreException {
		...
		try {
            ...
            IRunCommand rc = debuggerID!=null?CDebugCorePlugin.getDefault().getRunCommand(debuggerID):null;
...
//          To capture errors early
			...
			boolean resume = config.getAttribute(ICDTLaunchConfigurationConstants.ATTR_DEBUGGER_RESUME_AT_START, ICDTLaunchConfigurationConstants.DEBUGGER_RESUME_AT_STARTUP_DEFAULT); // CUSTOMIZATION
			...
			for( int i = 0; i < targets.length; i++ ) {
				...
				CDIDebugModel.newDebugTarget( launch, project.getProject(), targets[i], renderTargetLabel( debugConfig ), iprocess, exeFile, true, false, stopSymbol,  resume /*CUSTOMIZATION*/ );
			}
		}
