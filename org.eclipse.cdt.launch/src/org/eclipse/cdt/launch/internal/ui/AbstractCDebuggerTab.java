...
public abstract class AbstractCDebuggerTab extends CLaunchConfigurationTab {
	...
	protected void loadDebuggerCombo(ICDebugConfiguration[] debugConfigs,
			String current) {
		int select = -1;
		// <CUSTOMIZATION> prevent unnecessary regen of debug page
		boolean regen = false;
		String[] items = fDCombo.getItems();
		if (items.length != debugConfigs.length) {
			regen = true;
		} else {
			for (int i = 0; i < debugConfigs.length; i++) {
				if (fDCombo.getData(Integer.toString(i)) != debugConfigs[i]) {
					regen = true;
					break;
				}
		...
		// <CUSTOMIZATION>
		// An undesirable feature of the current scheme is that the debugger
		// must be set in addition to the program name. Just always select the
		// correct debugger.
		if (select < 0)
			select = 0;
		// </CUSTOMIZATION>
	...
	public boolean canSave(){
	    if (!super.canSave()) return false;
	    if (fDynamicTab != null){
	        return fDynamicTab.canSave();
	    }
	    return true;
	}
