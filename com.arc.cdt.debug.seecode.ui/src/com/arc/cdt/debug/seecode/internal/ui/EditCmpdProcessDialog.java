...
class EditCmpdProcessDialog extends Dialog {
    ...
    public EditCmpdProcessDialog(Shell parent) {
        ...
        if (command != null) {
            ...
            if (project != null && !exePath.isAbsolute()) {
                if (!project.getFile(command).exists()) {
                    if (complain) {
                        error(LaunchMessages.getString("CMainTab.Program_does_not_exist")); //$NON-NLS-1$
            ...
            try {
                if (!isBinary(project, exePath)) {
                    if (complain) {
                        error(LaunchMessages.getString("CMainTab.Program_is_not_a_recongnized_executable")); //$NON-NLS-1$
                        fPathSelector.setFocus();
                    }
                    return false;
                }
